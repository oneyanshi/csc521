...
def FunctionDeclaration0(pt, scope):
    ...
    '''
    name = func_by_name(pt[1][0], pt[1], scope)
    params = func_by_name(pt[3][0], pt[3], scope)
    # func_body = func_by_name(pt[5][0], pt[5], scope)
    scope[name] = [params + ]
...
def SingleAssignment0(pt, scope):
    ...
    #bonus Print error message if name was already bound in scope.
    '''
    return -1
...
def Factor2(pt, scope):
    # <FunctionCall>
    func_by_name(pt[1][0], pt[1], scope, scope)[0]
...
e4tree = ["Program1", ["Statement2", ["Print0", "PRINT", ["Expression2", ["Term2", ["Factor0", ["SubExpression0", "LPAREN", ["Expression2", ["Term0", ["Factor4", ["Value1", ["Number0", "NUMBER:4"]]], "MULT",
            ["Term2", ["Factor4", ["Value1", ["Number0", "NUMBER:3"]]]]]], "RPAREN"], "EXP", ["Factor4", ["Value1", ["Number0", "NUMBER:2"]]]]]]]]]
